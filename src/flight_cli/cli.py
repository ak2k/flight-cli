...
def _parse_times(s: str | None) -> tuple[TimeOfDay, ...]:
    ...
    out: list[TimeOfDay] = []
    aliases = {
        "early": TimeOfDay.EARLY_MORNING, "early_morning": TimeOfDay.EARLY_MORNING,
        "morning": TimeOfDay.MORNING, "midday": TimeOfDay.MIDDAY,
        "noon": TimeOfDay.MIDDAY, "afternoon": TimeOfDay.AFTERNOON,
        "evening": TimeOfDay.EVENING, "night": TimeOfDay.NIGHT,
    }
    for raw in s.split(","):
        key = raw.strip().lower().replace("-", "_")
        if key not in aliases:
            err.print(f"[red]bad time-of-day {raw!r}; choose: "
...
def _resolve_cabin(name: str) -> Cabin:
    ...
    aliases = {
        "coach": Cabin.COACH, "economy": Cabin.COACH, "y": Cabin.COACH,
        "premiumcoach": Cabin.PREMIUM_COACH, "premiumeconomy": Cabin.PREMIUM_COACH,
        "premium": Cabin.PREMIUM_COACH, "w": Cabin.PREMIUM_COACH,
        "business": Cabin.BUSINESS, "j": Cabin.BUSINESS,
        "first": Cabin.FIRST, "f": Cabin.FIRST,
    }
    if norm in aliases:
        return aliases[norm]
...
def _build_options(*, cabin: str, adults: int, children: int, seniors: int,
                    youth: int, infants_in_seat: int, infants_in_lap: int,
                    stops: int | None, allow_airport_changes: bool,
                    show_only_available: bool,
                    page_size: int = 25) -> SearchOptions:
...
def _run(search: Search, rps: float, impersonate: str, no_cache: bool):
    async def go():
...
def _render_search(res: SearchResult) -> None:
    ...
    console.print(f"[bold]{res.solution_count} solutions[/]  · "
...
def _render_calendar(res: CalendarResult, *, dmin: int, dmax: int,
                      origin: tuple[str, ...], destination: tuple[str, ...],
                      sd: date, ed: date) -> None:
    if res.solution_count == 0 or not res.priced_days:
        console.print("[yellow]Calendar empty.[/] Matrix's calendar mode "
...
def fare(
    ...
    slice_specs: Annotated[Optional[list[str]], typer.Option(
        "--slice", "-s",
        help="Multi-city: 'ORIG-DEST:DATE[:r=ROUTING:e=EXT]'. Repeat.")] = None,
    cabin: str = "economy",
    adults: int = 1, children: int = 0, seniors: int = 0, youth: int = 0,
    inf_seat: Annotated[int, typer.Option("--inf-seat")] = 0,
    inf_lap: Annotated[int, typer.Option("--inf-lap")] = 0,
    routing: Annotated[Optional[str], typer.Option("--routing",
        help="Routing language ('LH+', 'BA AA', '[F* X F*]')")] = None,
    extension: Annotated[Optional[str], typer.Option("--extension", "--ext",
        help="Extension codes ('MAXCONNECT 2:00', 'MAXSTOPS 1')")] = None,
    depart_times: Annotated[Optional[str], typer.Option("--depart-times",
    ...
    only_available: bool = typer.Option(
        True, "--only-available/--include-unavailable"),
    page_size: int = typer.Option(10, "--n", "-n"),
    rps: float = _RPS_OPT, impersonate: str = _IMPERSONATE_OPT,
    ...
    if slice_specs:
        ...
        legs = (Leg.of(_parse_iata_list(origin), _parse_iata_list(destination),
                        _parse_date(dep),
                        route_language=routing, extension=extension,
                        time_ranges=out_times),)
        if ret:
            legs += (Leg.of(_parse_iata_list(destination), _parse_iata_list(origin),
                             _parse_date(ret),
                             route_language=routing, extension=extension,
    ...
    opts = _build_options(
        ...
        youth=youth, infants_in_seat=inf_seat, infants_in_lap=inf_lap,
        stops=stops, allow_airport_changes=allow_airport_changes,
        show_only_available=only_available, page_size=page_size,
    )
...
def _parse_slice_spec(s: str) -> Leg:
    ...
    if len(parts) < 2:
        raise typer.BadParameter(
            f"slice {s!r} should be ORIGIN-DEST:DATE[:r=...:e=...]")
    od, dt = parts[0], parts[1]
    o, d = od.split("-", 1)
    routing = extension = None
    if len(parts) == 3:
        for chunk in re.split(r":(?=[re]=)", parts[2]):
            if chunk.startswith("r="): routing = chunk[2:]
            elif chunk.startswith("e="): extension = chunk[2:]
    return Leg.of(o, d, _parse_date(dt),
                   route_language=routing, extension=extension)
...
def calendar(
    ...
    cabin: str = "economy",
    adults: int = 1, children: int = 0, seniors: int = 0, youth: int = 0,
    routing: Optional[str] = typer.Option(None, "--routing"),
    extension: Optional[str] = typer.Option(None, "--extension", "--ext"),
    routing_return: Optional[str] = typer.Option(None, "--routing-ret"),
    extension_return: Optional[str] = typer.Option(None, "--ext-ret"),
    depart_times: Optional[str] = typer.Option(None, "--depart-times"),
    return_times: Optional[str] = typer.Option(None, "--return-times"),
    stops: Optional[int] = typer.Option(None, "--stops"),
    allow_airport_changes: bool = typer.Option(
        True, "--allow-airport-changes/--no-airport-changes"),
    only_available: bool = typer.Option(
        True, "--only-available/--include-unavailable"),
    ...
    out_leg = Leg.of(origins, dests, route_language=routing,
                      extension=extension, time_ranges=out_times)
    legs = (out_leg,)
    if not one_way:
        legs += (Leg.of(dests, origins,
    ...
    _render_calendar(res, dmin=dmin, dmax=dmax,
                      origin=origins, destination=dests, sd=sd, ed=ed)
    _emit_urls(search, matrix_url=matrix_url, google_url=google_url)
...
def detail(
    ...
    end: Annotated[Optional[str], typer.Option("--end",
        help="Original calendar window end (defaults to start+30d)")] = None,
    duration: Annotated[str, typer.Option("--duration", "-d",
        help="Original duration range")] = "5-7",
    cabin: str = "economy",
    adults: int = 1, children: int = 0, seniors: int = 0, youth: int = 0,
    routing: Optional[str] = typer.Option(None, "--routing"),
    extension: Optional[str] = typer.Option(None, "--extension", "--ext"),
    ...
    legs = (Leg.of(origins, dests, dep_d,
                    route_language=routing, extension=extension),)
    if ret_d:
        legs += (Leg.of(dests, origins, ret_d,
                         route_language=routing_return or routing,
                         extension=extension_return or extension),)
...
def gflight(
    ...
    from .fli_bridge import run_gflight_search
    legs = (Leg.of(origin, destination, _parse_date(dep)),)
    if ret:
        legs += (Leg.of(destination, origin, _parse_date(ret)),)
    opts = _build_options(
        cabin=cabin, adults=adults, children=children, seniors=0, youth=0,
        infants_in_seat=0, infants_in_lap=0,
        stops=None, allow_airport_changes=True, show_only_available=True,
    )
    search = SpecificDateSearch(legs=legs, options=opts)

    try:
        results = run_gflight_search(search, top_n=top_n)
    except Exception as e:
    ...
    t = Table(title=f"Google Flights · {origin.upper()}→{destination.upper()}"
              + (f" + return" if ret else ""),
    ...
    for i, r in enumerate(results[:top_n], 1):
        items = list(r) if isinstance(r, tuple) else [r]
        for j, fr in enumerate(items):
            label = f"{i}{'a' if j == 0 else 'b'}" if len(items) > 1 else str(i)
            legs_str = " → ".join(
                f"{getattr(leg.airline,'name',leg.airline)} "
                f"{getattr(leg,'flight_number','?')}"
                for leg in fr.legs
            )
            mins = fr.duration
            dur = f"{mins//60}h{mins%60:02d}m"
            t.add_row(label, f"{fr.currency or 'USD'}{fr.price:.2f}",
                       str(fr.stops), dur, legs_str)
    console.print(t)
...
def airport(
    ...
    locs = asyncio.run(go())
    if not locs:
        console.print("[yellow]No matches.[/]"); return
    t = Table(title=f"Airport lookup: {query!r}", show_header=True,
               header_style="bold blue")
    t.add_column("code"); t.add_column("name"); t.add_column("city"); t.add_column("tz")
    for loc in locs:
        t.add_row(loc.code, loc.display_name or "", loc.city_name or "",
                   loc.timezone or "")
    console.print(t)
