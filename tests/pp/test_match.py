...
from flight_cli.models import (
    Itinerary, ItineraryDetails, SearchResult, Slice, SliceEndpoint,
)
from flight_cli.pp.match import (
    award_match_key, cash_match_key, join,
)
...
def _itin(*slices_data: tuple[str, str, str, str]) -> Itinerary:
    ...
    slcs = [
        Slice(
            flights=[fn], departure=dep,
            origin=SliceEndpoint(code=o), destination=SliceEndpoint(code=d),
        )
...
def _award(fn: str, dep: str, *, miles: int = 47000, tax: float = 250.0,
           cabin: str = "Economy", origin: str = "EWR", dest: str = "LHR"
           ) -> OutboundFlight:
...
def test_join_codeshare_surfaces_multiple_airlines():
    """When two airlines (PP-side) report the same flight#×date as their own
    award, both options should attach to the cash itinerary."""
    res = SearchResult(solutions=[
        _itin(("BA178", "2026-06-09T07:50:00", "JFK", "LHR")),
    ])
    award_by_airline = {
        "American": AirlineSearchResponse(outboundFlights=[
            _award("BA178", "2026-06-09T07:50:00", miles=30000, tax=308.0),
        ]),
        "British Airways": AirlineSearchResponse(outboundFlights=[
...
def test_join_use_inbound_reads_inboundFlights():
    ...
    matches = join(res, {"United": award_resp}, _pricing(),
                    slice_index=1, use_inbound=True)
